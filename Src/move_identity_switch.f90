...
SUBROUTINE Identity_Switch
   ...
   LOGICAL :: accept_or_reject !rotation variables
...
!
   ...
   xcom_i_old = molecule_list(lm_i,is)%xcom
   ycom_i_old = molecule_list(lm_i,is)%ycom
   zcom_i_old = molecule_list(lm_i,is)%zcom

   dx_xcom_i = xcom_i_old - atom_list(:,lm_i,is)%rxp
   dy_ycom_i = ycom_i_old - atom_list(:,lm_i,is)%ryp
   dz_zcom_i = zcom_i_old - atom_list(:,lm_i,is)%rzp

   xcom_j_old = molecule_list(lm_j,js)%xcom
   ycom_j_old = molecule_list(lm_j,js)%ycom
   zcom_j_old = molecule_list(lm_j,js)%zcom

   dx_xcom_j = xcom_j_old - atom_list(:,lm_j,js)%rxp
   dy_ycom_j = ycom_j_old - atom_list(:,lm_j,js)%ryp
   dz_zcom_j = zcom_j_old - atom_list(:,lm_j,js)%rzp
   ...
   molecule_list(lm_j,js)%xcom = xcom_i_old
   molecule_list(lm_j,js)%ycom = ycom_i_old
   molecule_list(lm_j,js)%zcom = zcom_i_old

   atom_list(:,lm_j,js)%rxp = molecule_list(lm_j,js)%xcom - dx_xcom_j
   atom_list(:,lm_j,js)%ryp = molecule_list(lm_j,js)%ycom - dy_ycom_j
   atom_list(:,lm_j,js)%rzp = molecule_list(lm_j,js)%zcom - dz_zcom_j
